...
function flattenCategoryTree(
  ...
  for (const node of nodes) {
    // Avoid listing the current category or its descendants as parents
    ...
    if (node.children && node.children.length > 0) {
      list = list.concat(flattenCategoryTree(node.children, depth + 1, currentPath, currentId));
    }
  }
  return list;
}
...
export default function EditCategoryPage({ params }: EditCategoryPageProps) {
  ...
  // Load Category Data, Tree, CMS Blocks, Products
  useEffect(() => {
    setLoading(true);

    Promise.all([
      fetch(`/api/admin/categories/${id}`).then((r) => r.json()),
      fetch("/api/admin/categories/tree?includeInactive=true").then((r) => r.json()),
      fetch("/api/admin/cms/blocks").then((r) => r.json()),
      fetch("/api/admin/products?limit=100").then((r) => r.json()),
    ])
      .then(([catRes, treeRes, blocksRes, prodsRes]) => {
        // Flatten category tree (excluding current category to avoid circular loop)
        const rawTree = treeRes.data?.categories || treeRes.data || [];
        if (Array.isArray(rawTree)) {
          setFlatCategories(flattenCategoryTree(rawTree, 0, "", id));
        }
        ...
        // Available products
        const prodList = prodsRes.data?.products || prodsRes.data?.items || prodsRes.data || [];
        if (Array.isArray(prodList)) {
          setAvailableProducts(prodList);
        }

        // Category Detail
        const c = catRes.data?.category || catRes.data;
        if (c) {
          setName(c.name || "");
          setSlug(c.slug || "");
          setParentId(c.parentId || "");
          setIsActive(c.isActive ?? true);
          setIncludeInMenu(c.includeInMenu ?? true);
          setSortOrder(String(c.sortOrder ?? 0));
          setCustomLayout(c.customLayout || "DEFAULT");
          setDescription(c.description || "");
          setDisplayMode(c.displayMode || "BOTH");
          setBannerUrl(c.bannerUrl || "");
          setIconUrl(c.iconUrl || c.imageUrl || "");

          if (Array.isArray(catRes.data?.breadcrumbs)) {
            setBreadcrumbs(catRes.data.breadcrumbs);
          }

          if (Array.isArray(c.blocks)) {
            setSelectedBlockIds(c.blocks.map((b: any) => b.blockId || b.block?.id));
          }

          if (Array.isArray(c.products)) {
            setSelectedProductIds(c.products.map((p: any) => p.productId || p.product?.id));
          }

          if (c.seo) {
            setMetaTitle(c.seo.metaTitle || "");
            setMetaDescription(c.seo.metaDescription || "");
            setMetaKeywords(c.seo.metaKeywords || "");
            setCanonicalUrl(c.seo.canonicalUrl || "");
          }
        }
      })
      .catch((err) => {
  ...
  const handleSave = async (stayOnPage = false) => {
    ...
    try {
      ...
      setSuccessMsg("Category landing settings saved successfully.");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push("/admin/catalogue/categories");
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="text-xs text-white-chalk-100/50">Loading category studio...</p>
      </div>
    );
  }
              ...
              <div className="bg-[#161b22] mt-6 border border-white-chalk-100/10 rounded-2xl p-6 shadow-xl space-y-4">
                ...
                <div className="pt-2">
                  ...
                  <p className="text-[10px] text-white-chalk-100/40 mt-1">
                    Current Breadcrumbs:{" "}
                    <strong className="text-white-chalk-100">
                      {breadcrumbs.map((b) => b.name).join(" > ") || name}
                    </strong>
                  </p>
                </div>
      ...
      {deleteModalOpen && (
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-[11px] text-white-chalk-100/40">
              Any products assigned to this category will remain in the catalogue. Subcategories will be moved to root or parent level.
            </p>
