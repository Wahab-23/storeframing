  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-matt-black-100">Brands</h2>
          <p className="text-sm text-matt-black-300 mt-1">Manage your catalogue brands</p>
        </div>
        <Link 
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-matt-black-500/10">
            {loading ? (
              ...
              brands.map((brand) => (
                <tr key={brand.id} className="hover:bg-white-chalk-100/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-matt-black-100">{brand.name}</td>
                  <td className="px-6 py-4 text-matt-black-300">{brand.slug}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${brand.isActive ? 'bg-pablano-500 text-pablano-100' : 'bg-matt-black-500 text-matt-black-200'}`}>
